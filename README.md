...
This repository provides the DECRO dataset mentioned in the paper: Transferring Audio Deepfake Detection Capability across Languages (TheWebConf 2023). **DE**epfake **CRO**ss-lingual evaluation dataset is constructed to evaluate the influence of language difference. 

## Composition
DECRO consists of two subsets: English and Chinese subsets. The English and Chinese parts both contain bona-fide and spoofed speech samples, and have almost the same total audio length. Most importantly, the spoofed speech signals in the two parts are generated with the same types of synthetic techniques, which helps to exclude other interference factors and \qw{benefits accurate measurement} on the detection accuracy.

### Composition of Bona-fide Part
There are 21218 bona-fide utterances in the Chinese subset and 12484 bona-fide utterances in the English subset. 

For Chinese bona-fide speech, we collected data from multiple open-sourced recording datasets. Six recording datasets are covered to guarantee the diversity of Chinese recordings, including Aidatatang\_200zh, Aishell1, Aishell3, freeST, MagicData, and Aishell2 (free for academic usage). 

For the English part, bona-fide audios are collected from the ASVspoof2019 LA dataset and redivided to fit our setting.

### Composition of Spoofed Part
There are 41880 and 42800 spoofed utterances in the Chinese and the English dataset, respectively. 

Part of the spoofed speeches is from public datasets and the others are generated using commercial TTS and open-source algorithms. Although Wavefake and FAD are datasets based on different languages, they collect some speech samples generated by the same synthesis algorithms, including HiFiGAN, Multiband-MelGAN and PWG. 

Besides, Tacotron, FastSpeech2, VITS and StarGANv2 are end-to-end synthesis algorithms inherently supporting the generation of both Chinese and English. We collect Tacotron English data from A10 in the ASVspoof2019 LA dataset and generate the others with the corresponding pre-trained models. Note that NVCNet was initially proposed to perform VC in English. We retrain the model to generate Chinese speech. The Chinese part of Baidu and Xunfei are from FMFCC-A dataset and the English part is synthesized by ourselves via online APIs. _Noted, we refer to spoofing algorithms using abbreviations in the latter section._
...
## Data Format
The distributed audio files are encoded at 1 channel, *.wav format. The corpus is split into train, dev, and eval subsets. 

## Protocol Format
All protocol files are in ASCII format for deepfake detection models. Each column of the protocol is formatted as:

> SPEAKER_ID AUDIO_FILE_NAME - SYSTEM_ID KEY

where,
1. SPEAKER_ID:   ****, speaker ID
2. AUDIO_FILE_NAME:  ****, name of the audio file (!!!! no file extension, eg. "001" for "001.wav")
...
## Future Plan
1. **2023.2.10 -- 2024.1.1** More spoofing algorithms will be used for generating Chinese and English data.
2. **2024.1.1 -- 2026.1.1** Addition languages will be added.
...
## Contact 
Qing Wen at the Zhejiang University in HangZhou (qwerwenqing@icloud.com)